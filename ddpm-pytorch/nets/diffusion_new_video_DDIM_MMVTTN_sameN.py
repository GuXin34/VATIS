...
from nets.denoising_MMV import generalized_steps
import tqdm
...
class GaussianDiffusion(nn.Module):
    ...
    def get_losses(self, x,x_f,y,video):
        ...
        t_f = torch.randint(
            low=998, high=self.num_timesteps, size=(n // 2 + 1,)
        ).to(device)
        ...
        perturbed_xf = x_f * a_f.sqrt() + e * (1.0 - a_f).sqrt()
        
        estimated_noise = self.model(perturbed_x, perturbed_xf,t.float(),t_f.float(), y,video)
