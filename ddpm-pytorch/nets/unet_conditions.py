...
class AttentionBlock(nn.Module):
    def __init__(self, in_channels, norm="gn", num_groups=32):
        ...
        #self.to_k = nn.Conv2d(1, in_channels , 1)
        

    def forward(self, x,video):
        b, c, h, w  = x.shape
        if video.shape[1]<h*w:
            video=video.view(b,h,-1)
            video = video.repeat(1,1, int(w/video.shape[-1])).unsqueeze(1)
            video = video.repeat(1,c, 1,1)
        else:
            video=video.view(b,-1,h,w)
            video = video.repeat(1, int(c/video.shape[1]),1,1)
                
        

        
        #print(video.shape)
        #k = self.to_k(video).view(b,c,h*w)
        k = video.view(b,c,h*w)
