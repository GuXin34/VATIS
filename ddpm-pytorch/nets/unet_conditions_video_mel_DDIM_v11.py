...
class AttentionBlock(nn.Module):
    def __init__(self, in_channels, norm="gn", num_groups=32):
        ...
        #self.v_l = nn.Linear(512, 100)#-----------v7-2----------
...
class UNet(nn.Module):
    def __init__(
        ...
        # 对视频轴输入的全连接层
        self.video_music = nn.Sequential(

            nn.Linear(video_dim, 80),
            SiLU(),
...
#-------------------------------------------------video在最初的时候加上，图片先卷积后加video-----------------------------------        
        video_emb=self.video_music(video)
        #print(video_emb.shape)
        video_emb=video_emb.unsqueeze(-2).repeat(1,1,80,1)
