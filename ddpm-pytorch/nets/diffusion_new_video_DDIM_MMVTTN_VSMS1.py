...
from nets.denoising_VSMS import generalized_steps
import tqdm
...
class GaussianDiffusion(nn.Module):
    ...
    def get_losses(self, x,x_now,y,video):
        ...
        #t_f = torch.randint(low=998, high=self.num_timesteps, size=(n // 2 + 1,)).to(device)
        #t_f = torch.cat([t_f, self.num_timesteps - t_f - 1], dim=0)[:n]
        ...
        #perturbed_xf = x_f * a_f.sqrt() + e2 * (1.0 - a_f).sqrt()
        
        estimated_noise,video_emb = self.model(perturbed_x,None,t.float(), y,video)
        

        if self.loss_type == "l1":
            loss1 = F.l1_loss(estimated_noise, e)
            loss2 = F.l1_loss(x, video_emb)
            loss =loss1+loss2
        elif self.loss_type == "l2":
            loss1 = F.mse_loss(estimated_noise, e)
            loss2 = F.mse_loss(x, video_emb)
            loss = loss1+loss2
        return loss

    def forward(self, x, x_now=None,y=None,video=None):
        b, c, h, w  = x.shape
        device      = x.device

        
        #t = torch.randint(0, self.num_timesteps, (b,), device=device)#self.num_timesteps  = len(betas)
        return self.get_losses(x, None,y,video)
