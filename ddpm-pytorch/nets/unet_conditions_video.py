...
#   下采样层，一个步长为2x2的卷积
#------------------------------------------#
class Downsample(nn.Module):#大小减半
    def __init__(self, in_channels):
        super().__init__()

        self.downsample = nn.Conv1d(in_channels, in_channels, 3, stride=2, padding=1)
...
class AttentionBlock(nn.Module):
    def __init__(self, in_channels, norm="gn", num_groups=32):
        ...
        self.to_qv = nn.Conv1d(in_channels, in_channels*2 , 1)
        self.to_out = nn.Conv1d(in_channels, in_channels, 1)
        self.to_k80 = nn.Linear(512, in_channels*80)
        self.to_k20 = nn.Linear(512, in_channels*20)
        

    def forward(self, x,video):
        b, c, h  = x.shape
        if h==80:
            video = self.to_k80(video)
        elif h==20:
            video = self.to_k20(video)
        ...
        assert dot_products.shape == (b, h , h )
...
class ResidualBlock(nn.Module):
    def __init__(
        ...
        norm="gn", num_groups=32, use_attention=False,video_dim=None,
    ):
        super().__init__()
        ...
        self.conv_1 = nn.Conv1d(in_channels, out_channels, 3, padding=1)
        ...
        self.residual_connection    = nn.Conv1d(in_channels, out_channels, 1) if in_channels != out_channels else nn.Identity()
    ...
    def forward(self, x, time_emb=None, y=None,video=None):
        ...
        if self.time_bias is not None:
            ...
            out += self.time_bias(self.activation(time_emb))[:, :, None]

            
            # 对视频信息video做一个全连接，施加在通道上
            #out += self.video_bias(self.activation(video))[:, :, None, None]
...
class UNet(nn.Module):
    def __init__(
        ...
        dropout=0.1, attention_resolutions=(1,), norm="gn", num_groups=8, initial_pad=0,base_video_channels=2048,video_dim=2048
    ):
        super().__init__()
        # 使用到的激活函数，一般为SILU
        self.activation = activation
        ...
        self.video_mlp = nn.Sequential(
            ...
            nn.Linear(base_channels, video_dim),
            SiLU(),
            nn.Linear(video_dim, video_dim),
        )
    
        # 对输入图片的第一个卷积
        self.init_conv  = nn.Conv1d(img_channels, base_channels, 3, padding=1)
        ...
        self.out_conv = nn.Conv1d(base_channels, img_channels, 3, padding=1)
