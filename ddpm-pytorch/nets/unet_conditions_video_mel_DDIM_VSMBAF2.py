...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x,x_f,video):
        b, c, h, w  = x.shape
        q, v     = torch.split(self.to_qv(self.norm(x_f)), self.in_channels, dim=1)
      
        kf = x.permute(0, 1, 2, 3).view(b, c, h * w)
...
# 再次交换轴，将通道数放回最后
        ...
        x_f,_ = self.attention(video_emb,x_t,None)
        
        x_f = x_f+ self.init_conv(x_now)
