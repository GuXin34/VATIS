...
class AttentionBlock(nn.Module):
    def __init__(self, in_channels, norm="gn", num_groups=32):
        ...
        self.v_c =  nn.Conv1d(1, in_channels, 1)
        self.v_l = nn.Linear(512, 100)
        
        self.to_out = nn.Conv2d(in_channels, in_channels, 1)
        

    def forward(self, x,x_f,Flag=False):
        b, c, h, w  = x.shape
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, x_f,time_emb=None, y=None,video=None):
        ...
        out,_ = self.attention(out,out,False)
        out,out_f = self.attention(out,out_f,False)
        out,_ = self.attention(out,video,True)
...
#------------------------------------------------------------------------------------
        ...
        for layer in self.downs:
            x,x_f= layer(x, x_f,time_emb, y,v)
            skips.append(x)
            skips_f.append(x_f)
