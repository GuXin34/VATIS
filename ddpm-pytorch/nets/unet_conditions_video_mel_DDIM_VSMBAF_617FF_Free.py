...
class Downsample(nn.Module):
    ...
    def forward(self, x, x_f,time_emb, y, video):
        ...
        return self.downsample(x),self.downsample(x_f),self.downsample(video)
...
class Upsample(nn.Module):
    ...
    def forward(self, x, x_f,time_emb, y,video):
        return self.upsample(x),self.upsample(x_f),self.upsample(video)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x,x_f,video):
        b, c, h, w  = x.shape
        k, v     = torch.split(self.to_kv(self.norm(x)), self.in_channels, dim=1)
        qf = self.to_q(self.norm(x_f))
        qf = qf.permute(0, 1, 2, 3).view(b, c, h * w)
        #vf = x_f.permute(0, 2, 3, 1).view(b, h * w, c)
        if video is not None:
          
            qv = self.to_q(self.norm(video))
            qv = qv.permute(0, 1, 2, 3).view(b, c, h * w)
        ...
        dot_products1 = torch.bmm(k, qf) * (c ** (-0.5))
        
        if video is not None:
          dt2 = torch.bmm(k, qv) * (c ** (-0.5))
          dot_products1 = 0.5*(dot_products1 + dt2)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, x_f,time_emb=None, y=None,video=None):
        ...
        out_video = self.activation(self.norm_1(video))
        # 第一个卷积
        out_video = self.conv_1(out_video)
        ...
        out_video = self.activation(self.norm_2(out_video))
        # 第二个卷积+残差边
        out_video = self.conv_2(out_video) + self.residual_connection(video)
        
        out,_ = self.attention(out,out,None)
        #out,out_video = self.attention(out,out_video)
        out,out_f = self.attention(out,out_f,out_video)
        
        return out,out_f,out_video
...
# 再次交换轴，将通道数放回最后
        ...
        h = x.type(self.dtype)
        h_f = x_f.type(self.dtype)
        h_v = v.type(self.dtype)
        skips = [h]
        skips_f = [h_f]
        skips_v = [h_v]
        
        for layer in self.downs:
            h,h_f,h_v = layer(h, h_f,time_emb, y,h_v)
            skips.append(h)
            skips_f.append(h_f)
            skips_v.append(h_v)
        ...
        for layer in self.ups:
            if isinstance(layer, ResidualBlock):
                ...
                if h.shape[-2] == 10:
                    ...
                    h[:,:c//2] = h[:,:c//2] * ((self.b1 - 1 ) * hidden_mean + 1)
                ...
                if h.shape[-2] == 20:
                    ...
                    h[:,:c//2] = h[:,:c//2] * ((self.b2 - 1 ) * hidden_mean + 1)
                    hs_ = Fourier_filter(hs_, threshold=1, scale=self.s2)
                
                h = torch.cat([h, hs_], dim=1)
                #x = torch.cat([x, skips.pop()], dim=1)
                h_f = torch.cat([h_f, skips_f.pop()], dim=1)
                h_v = torch.cat([h_v, skips_v.pop()], dim=1)
            h,h_f,h_v = layer(h,h_f, time_emb, y,h_v)

        h = h.type(x.dtype)
        # 上采样并进行特征融合
        h= self.activation(self.out_norm(h))
        h= self.out_conv(h)
        
        if self.initial_pad != 0:
            return h[:, :, ip:-ip, ip:-ip]
        else:
            return h,video_at