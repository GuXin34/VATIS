...
from nets.denoising_mel import generalized_steps
import tqdm
...
class GaussianDiffusion(nn.Module):
    ...
    def sample(self,batch_size, device,t, y=None, use_ema=True):
        ...
        with torch.no_grad():
            for _ in tqdm.tqdm(
                ...
                x = self.sample_image(x, self.ema_model,timesteps)
                #x = inverse_data_transform(config, x)
        return x            
    def sample_image(self, x, model, timesteps=1000,last=True):
    ...
    def get_losses(self, x,y):
        ...
        estimated_noise = self.model(perturbed_x, t.float(), y)
