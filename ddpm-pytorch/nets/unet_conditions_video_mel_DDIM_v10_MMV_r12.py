...
class Downsample(nn.Module):
    ...
    def forward(self, x, x_f,time_emb, time_embf, y, video):
        if x.shape[2] % 2 == 1:
...
class AttentionBlock(nn.Module):
    def __init__(self, in_channels, norm="gn", num_groups=32):
        ...
        self.v_c =  nn.Conv1d(16, in_channels, 1)
    ...
    def forward(self, x,x_f,video):
        ...
        k2 = x_f.permute(0, 1, 2, 3).view(b, c, h * w)
        ...
        dot_products2 = torch.bmm(q, k2) * (c ** (-0.5))
        
        dot_products = dot_products + dot_products2
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, x_f,time_emb=None, time_embf=None, y=None,video=None):
        ...
        if self.time_bias is not None:
            ...
            out_f += self.time_bias(self.activation(time_embf))[:, :, None, None]
        ...
        out = self.attention(out,out_f,video)
        return out
...
class UNet(nn.Module):
    ...
    def forward(self, x, x_f,time=None, time_f=None, y=None,video=None):
        # 是否对输入进行padding
        ip = self.initial_pad
        if ip != 0:
            x = F.pad(x, (ip,) * 4)
        ...
        temb_f = get_timestep_embedding(time_f, self.base_channels)
        temb_f = self.temb.dense[0](temb_f)
        temb_f = nonlinearity(temb_f)
        time_embf = self.temb.dense[1](temb_f)
#------------------------------------------------------------------------------------
        ...
        # 对输入图片的第一个卷积
        x = self.init_conv(x)
        x_f = self.init_conv(x_f)

        # skips用于存放下采样的中间层
        skips = [x]
        skips_f = [x_f]
        for layer in self.downs:
            x,x_f = layer(x,x_f,time_emb,time_embf,y,video_emb)
            skips.append(x)
            skips_f.append(x_f)
