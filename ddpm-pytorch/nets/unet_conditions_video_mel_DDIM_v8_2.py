#------修改--------------------
"""
num_groups=8  ----> num_groups=4
删掉self.to_qkv = nn.Conv2d(in_channels, in_channels, 1)
添加k = self.norm(k)
"""
import math
...
class AttentionBlock(nn.Module):
    def __init__(self, in_channels, norm="gn", num_groups=32):
        ...
        #self.to_qkv = nn.Conv2d(in_channels, in_channels, 1)
        
        self.v_c =  nn.Conv1d(1, in_channels, 1)#-----------v7-2----------
        self.v_1 = nn.Linear(512, 80*80)
        self.v_2 = nn.Linear(512, 40*40)
        self.v_3 = nn.Linear(512, 20*20)
        self.v_4 = nn.Linear(512, 10*10)
    ...
    def forward(self, x,video):
        ...
        k = k.repeat(1, self.in_channels, 1)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x, time_emb=None, y=None,video=None):
        ...
        out += self.video_bias(self.activation(video.squeeze()))[:, :, None, None]
...
class UNet(nn.Module):
    def __init__(
        ...
        dropout=0.1, attention_resolutions=(0,1,2,3,), norm="gn", num_groups=4, initial_pad=0,base_video_channels=512,video_dim=512
    ):
        super().__init__()
        # 使用到的激活函数，一般为SILU
        self.activation = activation
