...
class UNet(nn.Module):
    def __init__(
        ...
        self.Vconv1x1 = nn.Conv2d(in_channels=512, out_channels=88, kernel_size=1, stride=1, padding=0)
...
# 再次交换轴，将通道数放回最后
        ...
        for layer in self.downs:
            x,x_f,v = layer(x, x_f,time_emb, y,v)
            skips.append(x)
            skips_f.append(x_f)
            skips_v.append(v)
        ...
        for layer in self.ups:
            if isinstance(layer, ResidualBlock):
                ...
                v = torch.cat([v, skips_v.pop()], dim=1)
            x,x_f,v = layer(x,x_f, time_emb, y,v)
