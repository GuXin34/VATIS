...
from utils.utils import postprocess_output_mel, show_config,postprocess_output_mel_re
...
class Diffusion(object):
    _defaults = {
        ...
        #-----------------------------------------------#
        "model_path"        : '/home/vatis/DataDisk_1/23_vatis_PhD/guxin/Data/logs1/0416Diffusion_Epoch280-GLoss0.0062.pth',
        #-----------------------------------------------#
        #   卷积通道数的设置
        #-----------------------------------------------#
        "channel"           : 16,
    ...
    def generate(self):
        ...
        self.net    = GaussianDiffusion(UNet(4, self.channel), self.input_shape, 4)
    ...
    def generate_1x1_image(self, save_path='',video=None):
        with torch.no_grad():
            ...
            test_images = self.net.sample(video.size(0), randn_in.device, self.num_timesteps,None, use_ema=False,video=video)#最初video.size(0)是1
            ...
            for i in test_images:
                list1.append(np.concatenate(i, axis=1))
