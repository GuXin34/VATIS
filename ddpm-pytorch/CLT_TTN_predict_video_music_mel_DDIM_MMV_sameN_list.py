...
if __name__ == "__main__":
    ...
    def V_LT(video,n):
        ...
        video_LT = (video[n]+weighted_tensor).to(video[n].dtype)#
    ...
    for name in list1:
        ...
        video=preprocess_input_video(torch.tensor(video)[1:,:]).float()#全部帧
        
        
        m = np.load(f_m_root)#
        m=preprocess_input_mel(torch.tensor(m[:,:,:])).float()#全部帧
        print(m.shape)
    ...
    # 创建一个形状为 (3, 4) 的张量，填充标准正态分布的随机数
        ...
        for i in range(number):
            ...
            if i==0:
                music = m[:,:,:80].unsqueeze(0).to(device)
                #noise = noise_music(m[:,:,80*(i+1):80*(i+2)].unsqueeze(0),e)
            else:
                music = music_G[i-1].to(device)
                #noise = noise_music(m[:,:,80*(i+1):80*(i+2)].unsqueeze(0),e)
