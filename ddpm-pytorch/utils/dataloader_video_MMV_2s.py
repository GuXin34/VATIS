...
class DiffusionDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        len2=video.size(0)
        number= random.randint(2,min(len1,len2)-2)
        
        music = music_O[:,80*number:80*(number+2)]
        video = video[number:number+2,:]
        video = video.view(1,-1)
        
        music_f = music_O[:,80*(number-2):80*number]
