...
def fit_one_epoch(diffusion_model_train, diffusion_model, loss_history, optimizer,
    ...
    for iteration, items in enumerate(gen):
        ...
        with torch.no_grad():
            if cuda:
                ...
                video = video.cuda(local_rank)
                images_f = images_f.cuda(local_rank)
        
        if not fp16:
            optimizer.zero_grad()
            diffusion_loss = torch.mean(diffusion_model_train(images,images_f,None,video))
