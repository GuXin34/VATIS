...
if __name__ == "__main__":
    ...
    def preprocess_input_mel_Nor_V2(x):
        ...
        epsilon = 1e-8
        x = (x - means[:,np.newaxis]) / (std_devs[:,np.newaxis] + epsilon)
        return x
    
    def MM_melV2(x):
        x=(x + 6.46885724)/10.09142302
        x -= 0.5
        x /= 0.5
        return x
    ...
    def V_LT(video,n):
        ...
        return video_LT
    list1=['gWA_sBM_c02_d25_mWA2_ch10','gWA_sBM_c09_d25_mWA3_ch10','gPO_sBM_c03_d11_mPO0_ch09','gMH_sBM_c02_d24_mMH5_ch08','gLO_sBM_c09_d14_mLO0_ch09','gJS_sBM_c02_d03_mJS2_ch03','gJB_sBM_c09_d07_mJB3_ch06','gHO_sBM_c07_d19_mHO1_ch10','gBR_sBM_c06_d06_mBR4_ch06','gKR_sFM_c09_d28_mKR1_ch02']
    ...
    for name in list1:
        ...
        save_name = name+'VSMBFS12NorMM_POS_2T_new'+str(seed)+'.npy'
    ...
    # 创建一个形状为 (3, 4) 的张量，填充标准正态分布的随机数
        ...
        music=m[:,:,:80].unsqueeze(0).cuda()
        
        device      = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        #music = noise_music(m,x.cuda())
        noise = x.cuda()#torch.randn(1,1,80,80).cuda()#music
        #music = noise_music(music,noise)
