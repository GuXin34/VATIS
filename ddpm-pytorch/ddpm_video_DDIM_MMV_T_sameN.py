...
class Diffusion(object):
    _defaults = {
        ...
        #-----------------------------------------------#
        "model_path"        : '/home/vatis/DataDisk_1/23_vatis_PhD/guxin/Data/logs1/20241027HDiffusion_Epoch229-GLoss0.0099.pth',
        #-----------------------------------------------#
        #   卷积通道数的设置
        #-----------------------------------------------#
        "channel"           : 16,
    ...
    #   Diffusion1x1的图片
    #---------------------------------------------------#
    def generate_1x1_image(self,noise=None,music_f=None,video=None):
        with torch.no_grad():
            randn_in    = torch.randn((1, 1)).cuda() if self.cuda else torch.randn((1, 1))
            
            test_images = self.net.sample(video.size(0), randn_in.device, self.num_timesteps,None, use_ema=False,noise=noise,music_f=music_f,video=video)#最初video.size(0)是1
