...
import math
import random
from utils.utils_Normalization import cvtColor, preprocess_input_video,preprocess_input_mel,preprocess_input_mel_Nor_V2,MM_melV2
...
#'gWA_sBM_c02_d25_mWA2_ch10','gWA_sBM_c09_d25_mWA3_ch10','gPO_sBM_c03_d11_mPO0_ch09','gMH_sBM_c02_d24_mMH5_ch08','gLO_sBM_c09_d14_mLO0_ch09','gJS_sBM_c02_d03_mJS2_ch03','gJB_sBM_c09_d07_mJB3_ch06','gHO_sBM_c07_d19_mHO1_ch10','gBR_sBM_c06_d06_mBR4_ch06','gKR_sFM_c09_d28_mKR1_ch02']
    list4=['gHO_sBM_c09_d20_mHO1_ch06']    
    ...
    for N in range(NUMBER):
        ...
        save_path_N=os.path.join('/home/vatis/DataDisk_1/23_vatis_PhD/guxin/Aistt_DDIM_input',str(N)) 
        if not os.path.exists(save_path_N):
            os.makedirs(save_path_N)
    
        for name in list4:

            seed = 1401183600#random.randint(1,2**31-1)
            save_name = '2YN'+name+'VMBFS21_New_NorMM_'+str(seed)+'.npy'
            ...
            f_v_root=os.path.join('/home/vatis/DataDisk_1/23_vatis_PhD/guxin/Data/Aistt_Video_MetaClip',f_v_name)
            ...
            video =  positional_encoder(video.unsqueeze(0)).squeeze()
        ...
        # 创建一个形状为 (3, 4) 的张量，填充标准正态分布的随机数
            ...
            for j in range(3,-1,-1):
                ...
                video_s=V_LT(video,j).cuda()
                if j==3:
                    music_O=np.load(f_m_root).squeeze()
                    music = music_O[:,86*4:86*5]
                    music=torch.tensor(music)
                    music   = preprocess_input_mel_Nor_V2(music)
                    music  = MM_melV2(music).to(device)
                    music = torch.flip(music, dims=[-1]) 
                    zero_col = torch.zeros((music.size(0), 2), dtype=music.dtype, device=music.device)
					# 在开头添加全零列
                    music = torch.cat((zero_col, music), dim=1)
                    music = music.unsqueeze(0).unsqueeze(0)
                    music =noise_music(music,e)#music_G[i-1].to(device)                                                                                                                                                                                                                                                                                                                                                                                
                    g = ddpm.generate_1x1_image(noise,music,video_s.unsqueeze(0))
                    music_G.insert(0,g)
                else:
                    music =noise_music(music_G[0],e)#music_G[i-1].to(device)                                                                                                                                                                                                                                                                                                                                                                                
                    g = ddpm.generate_1x1_image(noise,music,video_s.unsqueeze(0))
                    music_G.insert(0,g)
            ...
            for i in range(4,number):
                ...
                if i==4:
                    music_O=np.load(f_m_root).squeeze()
                    music = music_O[:,86*5-88:86*5]
                    music=torch.tensor(music)
                    music   = preprocess_input_mel_Nor_V2(music)
                    music  = MM_melV2(music).unsqueeze(0).to(device)
                    
                    #video_s=V_LT(video,0).cuda()
                    #music = torch.zeros(1,1,80,88).to(device)
                    
                    music_G.append(music.unsqueeze(0))
                
                else:
                    video_s=V_LT(video,i).cuda()
                    music =noise_music(music_G[-1],e)#music_G[i-1].to(device)                                                                                                                                                                                                                                                                                                                                                                                
                    g = ddpm.generate_1x1_image(noise,music,video_s.unsqueeze(0))
                    music_G.append(g)
            ...
            for i in range(len(music_G)):
                ...
                if i==0:
                    ...
                    del list_E[-1]
                    list1.append(test_images[:,2:])
                    list_E.append(test_images[:,-2:])
                else:
                    list_S.append(test_images[:,:2])
                    A=(list_E[-1]+list_S[-1])/2
                    list1.append(A)
